...
public class HoleDetector : MonoBehaviour {
    ...
    public float holeDelta = 0.1f;
    public Transform volcanoPrefab;
    public int maxVolcanos = 3;
	...
	volcanos = new Transform[maxVolcanos];
	// Create Volcanos
	for (int i = 0; i < maxVolcanos; i++) {
	    Vector3 offscreen = new Vector3(100, 100, 100);
	    volcanos[i] = Instantiate(volcanoPrefab,
				      offscreen,
	...
	// Move all volcanos offscreen
	for (int i = 0; i < maxVolcanos; i++) {
	    Vector3 offscreen = new Vector3(100, 100, 100);
	    volcanos[i].position = offscreen;
	}
	...
	for (int y = 0; y < res.y; y++) {
		...
		MoveBox(box, 1 << layer);
		if (IsHole(x, y)) {
		    // Move volcanos back onscreen
		    box.renderer.material.color = Color.red;
		    volcanos[volcanoIndex].position = box.position;
		    volcanoIndex++;
		    if (volcanoIndex > maxVolcanos - 1) {
			volcanoIndex = maxVolcanos - 1;
		    }
		} else {
		    box.renderer.material.color = Color.grey; 
		}
	    }
	}
    }
	...
	Transform box = colliders[y * (int)res.x + x];
	Transform north = colliders[(y - 1) * (int)res.x + x];
	Transform south = colliders[(y + 1) * (int)res.x + x];
	Transform east = colliders[y * (int)res.x + x - 1];
	Transform west = colliders[y * (int)res.x + x + 1];
	return (IsWall(box, north) &&
		IsWall(box, south) &&
		IsWall(box, east) &&
		IsWall(box, west));
    }

    bool IsWall(Transform box, Transform other) {
	return (other.position.y - box.position.y >= holeDelta);
    }
