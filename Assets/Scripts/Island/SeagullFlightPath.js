...
function ReturnToPlayer()
{
	...
	while(distToTarget > 1)
	{
		...
		offset = Vector3(normal.x * 4, 0, normal.z * 4);
		
		flyHeight = (distToTarget > 8) ? highFlyHeight : lowFlyHeight;
...
function FlyAway()
{
	...
	while(time < awayTime)
	{
		away = player.position + (awayDir * 100);
		away.y = 0;
		
		toAway = away - transform.position;
		
		distToTarget = toAway.magnitude;
		if(distToTarget > 0) targetPos = transform.position + ((toAway / distToTarget) * 1);
		else targetPos = Vector3.zero;
...
function Move (delta : Vector3)
{
	...
	newPos = Vector3(myT.position.x + delta.x, 100, myT.position.z + delta.z);
	if(col.Raycast(Ray(newPos, -Vector3.up), hit, 200)) newHeight = hit.point.y;
	else newHeight = 0.00;
	if(newHeight < lastHeight) height = Mathf.Lerp(height, newHeight, flyDownSpeed * dTime);
	else height = newHeight;
	lastHeight = newHeight;
	myT.position = Vector3(newPos.x, Mathf.Clamp(height, 5.528, 100.00) + flyHeight, newPos.z);
}
