...
public class ProceduralTreeSegment : MonoBehaviour
{
	...
	//if true, the last node will only be a single non-rotated branch (written to incorporate Tat's tree pieces)
	public bool noBranchOnEndNode = false;
	...
	//DEVEL: delete this. it should only be in the scope of the Branch() method
	...
	{
		return transform.localPosition + transform.up.normalized * transform.localScale.y;
	}
	...
	private void setHasGrowthBehavior ()
	{
		...
		if (temp != null) {
			hasGrowthBehavior = true;
			//false because it's waiting for GrowOnBirthProcedural to set it to true
		} else {
